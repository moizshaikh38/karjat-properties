...
import { 
  LayoutDashboard, Inbox, Users, Calendar, Building2, 
  SlidersHorizontal, LogOut, Search, Sun, Moon, ChevronLeft, ChevronRight, Menu, X, CheckCircle2
} from 'lucide-react';
...
const NavItem: React.FC<NavItemProps> = ({ to, icon: Icon, label, badge, collapsed, onClick }) => (
  <NavLink
    ...
    onClick={onClick}
    className={({ isActive }) => `
      flex items-center px-2.5 py-1.5 rounded-[6px] transition-colors group relative select-none
      ${isActive 
  ...
  >
    ...
    {!collapsed && (
      <>
        <span className="ml-2.5 truncate text-[13px]">{label}</span>
        {badge !== undefined && badge > 0 && (
          <span className="ml-auto bg-[var(--color-accent)]/15 text-[var(--color-accent)] border border-[var(--color-accent)]/30 py-0.2 px-1.5 rounded-[3px] text-[11px] font-medium">
            {badge}
          </span>
        )}
      </>
    ...
    <div className="min-h-screen bg-[var(--color-bg)] flex flex-col md:flex-row overflow-hidden font-sans text-[var(--color-text)]">
      ...
      <header className="md:hidden flex items-center justify-between px-3.5 py-2.5 bg-[var(--color-surface)] border-b border-[var(--color-border)] sticky top-0 z-30">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="font-medium text-[13px] text-[var(--color-text)] tracking-tight block">Karjat Properties</span>
          </div>
        </div>
      ...
      `}>
        ...
        {/* Navigation Items (Streamlined 6 Items Max) */}
        <div className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5 hide-scrollbar">
          ...
          <NavItem to="/leads" icon={Users} label="Pipeline & Leads" collapsed={collapsed} />
          ...
          <NavItem to="/settings" icon={SlidersHorizontal} label="Settings" collapsed={collapsed} />
        </div>
      ...
      {mobileMenuOpen && (
        <div 
          className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-[2px] flex justify-end animate-in fade-in"
          onClick={() => setMobileMenuOpen(false)}
        >
          <div 
            className="w-3/4 max-w-xs bg-[var(--color-surface)] h-full flex flex-col border-l border-[var(--color-border)] shadow-2xl"
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              ...
              <NavItem to="/leads" icon={Users} label="Pipeline & Leads" collapsed={false} onClick={() => setMobileMenuOpen(false)} />
              ...
              <NavItem to="/settings" icon={SlidersHorizontal} label="Settings" collapsed={false} onClick={() => setMobileMenuOpen(false)} />
            </div>

            <div className="p-3 border-t border-[var(--color-border)] bg-[var(--color-surface-elevated)] space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-[var(--color-text-muted)]">{user?.email || 'admin@karjatproperties.com'}</span>
                <button
                  onClick={toggleTheme}
